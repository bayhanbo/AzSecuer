...
public class FileSearchManager{
    ...
    private void initDataList() {
        ...
        fileInfos.clear();
        fileInfos.put(FileSearchTypeEvent.TYPE_OTHER, new ArrayList<FileManagerInfo>());
        fileInfos.put(FileSearchTypeEvent.TYPE_APK, new ArrayList<FileManagerInfo>());
        fileInfos.put(FileSearchTypeEvent.TYPE_AUDIO, new ArrayList<FileManagerInfo>());
        fileInfos.put(FileSearchTypeEvent.TYPE_IMAGE, new ArrayList<FileManagerInfo>());
        fileInfos.put(FileSearchTypeEvent.TYPE_TXT, new ArrayList<FileManagerInfo>());
        fileInfos.put(FileSearchTypeEvent.TYPE_VIDEO, new ArrayList<FileManagerInfo>());
        fileInfos.put(FileSearchTypeEvent.TYPE_ZIP, new ArrayList<FileManagerInfo>());
        fileSizes.clear();
    ...
    public HashMap<String,ArrayList<FileManagerInfo>> getFileInfos() {
        return fileInfos;
    }
    ...
    public void startSearchFromInRom(Context context) {
        ...
        if (inRomPath != null) {
            ...
            LogUtil.d(getClass().getName(),"Search内置空间时,外置空间不存在");
            listener.onSearchEnd(true,0);
        }
    }
    /** 搜索外置空间所有文件 */
    public void startSearchFromOutRom(Context context) {
        ...
        if (outRomPath != null) {
            ...
            LogUtil.d(getClass().getName(),"Search外置空间时,外置空间不存在");
            listener.onSearchEnd(true, 1);
        }
    }
    /**
    ...
    private void fileSearch(File targetFileDir, int romType) throws Exception {
        ...
        if (targetFileDir.isFile()) {
            addFileToList(targetFileDir, romType); //todo 将此文件添加到文件集合
        }
        //todo 是目录
        if (targetFileDir.isDirectory()){
            ...
            if (files != null) {
                for (File file : files) {
                    fileSearch(file, romType); //todo 　递归文件搜索
                }
            }
        }
    }
    ...
    private void addFileToList(File file, int romType) {
        ...
        String fileType = iconTypes[2];//todo 文件类型(图像?音频?)
        String iconName = iconTypes[1];//todo 此文件对应所用图像名称
        String openType = FileSearchTypeEvent.getMIMEType(file);//todo 此文件打开类型
        FileManagerInfo fileInfo = new FileManagerInfo(file, romType, fileType, iconName, openType);
        //todo 将此文件添加到对应分类集合中
        fileInfos.get(fileType).add(fileInfo);
        //todo 将此类型文件总大小添加到对应集合
