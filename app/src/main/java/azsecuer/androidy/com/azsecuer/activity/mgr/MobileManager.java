...
public class MobileManager {
    ...
    //todo 是否同意指定权限(Manifest.permission.ACCESS_NETWORK_STATE)
    ...
    public ArrayList<MobileChildInfo> getMemoryMessage(Context context) {
        ...
        String inRom = NewMemoryManager.formatFileSize(NewMemoryManager.getTotalInternalMemorySize(),true);
        String outRom = NewMemoryManager.formatFileSize(NewMemoryManager.getTotalExternalMemorySize(),true);
        childs.add(new MobileChildInfo("最大RAM:", maxRam));
        childs.add(new MobileChildInfo("空闲RAM:", freeRam));
        childs.add(new MobileChildInfo("内置空间:", inRom));
        childs.add(new MobileChildInfo("外置空间:", outRom));
        return childs;
    }
    //todo 设备信息 -------------------------------------------------
